...
public class TestTaildirEventReader {
  ...
  private ReliableTaildirEventReader getReader(Map<String, String> filePaths,
    ...
    try {
      reader = new ReliableTaildirEventReader.Builder()
          ...
          .addInode(inode)
          .build();
      reader.updateTailFiles(Optional.<JSONObject>absent());
  ...
  public void testInitiallyEmptyDirAndBehaviorAfterReadingAll() throws IOException {
    ...
    List<Long> fileInodes = reader.updateTailFiles(Optional.<JSONObject>absent());
  ...
  public void testLoadPositionFile() throws IOException {
    ...
    ReliableTaildirEventReader reader = getReader();
    Map<Long, TailFile2> tailFiles = reader.getTailFiles();

    long pos = 2; // f2.length();
    int i = 1;
    ...
    for (TailFile2 tf : tailFiles.values()) {
      ...
      System.out.println(new String(tf.getFileReader().readRecord(), Charsets.UTF_8));
    }
  }
  ...
  public void testByteOffsetHeader() throws IOException {
    ...
    for (TailFile2 tf : reader.getTailFiles().values()) {
      headers = headersAsStrings(reader.readEvents(tf, 5), LINE_OFFSET_HEADER_KEY);
      reader.commit();
    }
