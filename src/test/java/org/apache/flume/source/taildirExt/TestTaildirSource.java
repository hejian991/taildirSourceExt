...
import static org.apache.flume.source.taildirExt.TaildirSourceConfigurationConstants.END_TIME;
...
import static org.apache.flume.source.taildirExt.TaildirSourceConfigurationConstants.HEADERS_PREFIX;
...
import static org.apache.flume.source.taildirExt.TaildirSourceConfigurationConstants.START_TIME;
import static org.apache.flume.source.taildirExt.TaildirSourceConfigurationConstants.LOG_FILE_TYPE;
...
//        String file = "/Users/hj/temp/b.11m";
//        String file = "/Users/hj/Downloads/sample_message.pb.log.201605171700";
        String file = "/Users/hj/code/project_baidu/avro-pb-test/php_seqfile_log_11m";
        ...
        context.put(LIMIT_RATE_HEADER, "20000");
        ...
        source.start();
        long timeStart = TimeUtil.currentTimeMs();
        source.process();
        System.out.println(TimeUtil.recordTime("source把11M数据全部成功推到SpillableMemoryChannel耗时：", timeStart));
        source.stop();
        ...
        System.out.println(TimeUtil.recordTime("从SpillableMemoryChannel 取出11M数据并加载到内存中耗时：", timeStart));
    ...
    // 需要 mv 一个老文件 到tmpDir 目录下，断点 测试 看是否filter文件 正确
    @Test
    public void testFileConsumeOrderAndTimeRange() throws IOException {
        System.out.println(tmpDir.toString());
        // 1) Create 1st file
        File f1 = new File(tmpDir, "file1");
        String line1 = "file1line1\n";
        ...
        Context context = new Context();
        context.put(POSITION_FILE, posFilePath);
        context.put(START_TIME, 1446134400000l + "");
        context.put(END_TIME, 1446220800000l + "");
        context.put(FILE_GROUPS, "g1");
        context.put(FILE_GROUPS_PREFIX + "g1", tmpDir.getAbsolutePath() + "/.*");
